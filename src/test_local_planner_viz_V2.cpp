...
struct SharedPath
{
    ...
    bool read(std::vector<Vec3>& out, uint64_t& outseq, double& age_ms)
    {
        std::lock_guard<std::mutex> lk(m);
        out = pts;
        outseq = seq;
        age_ms = std::chrono::duration<double, std::milli>(clk::now() - stamp).count();
        return valid;
    }
...
class PlanControlTestNode
{
    public: 
        PlanControlTestNode()
        {
            ...
            localPlanner.init(&validity_local_);

            globalRoute = loadRouteCSV(args.RouteCSV);
            if (globalRoute_.size() < 2)
            {
    ...
    private:
        ...
        std::atomic<double> followRadius_;
        std::atomic<double> speedCap;
    ...
    while (true) {
        ...
        if (!paused) {
            ...
            if (out.goal_reached) {
                ...
                vs.theta += (currentSpeed / pp.cfg_.wheelbase_m) * std::tan(out.steer_angle_rad) * dt;
            }
